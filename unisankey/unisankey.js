 ...
 * scale:           The scaling of this node when drawn.
...
UniSankeyNode.prototype.draw = function(context, set){
    ...
    if (this.type == 'percent' && EDITING == false){
    ...
    var old_colour = context.fillStyle;
    if (this.is_hidden > 0){
        context.fillStyle = getDarker(context.fillStyle, 0.7);
        context.strokeStyle = getDarker(context.fillStyle, 0.7);
    }
    ...
    context.fillStyle = old_colour;
    context.strokeStyle = old_colour;
    ...
    var max_height = height * MAX_HEIGHT;
    ...
    for (var i = 0; i < this.links.length; i++){
        ...
        if (this.type == 'percent' && EDITING == false){
            cur_height = cur_height * PERCENT_NODE_SCALING;
        }
        link_height_max += cur_height * MAX_HEIGHT;
    }

    current_y = this.y - link_height_max/2;
    var bottom_corner = top_corner + height * MAX_HEIGHT;

    if (current_y < top_corner){
        current_y = top_corner;
    }


    // Look for at least one link leading to this one that isn't hidden
    var non_hidden_previous_node = false;

    for (var i = 0; i < this.incoming_nodes.length; i++){
        var node = this.incoming_nodes[i];
        var incoming_links = node.links;
        for (var j = 0; j < incoming_links.length; j ++){

            var l = incoming_links[j];
            if (l.endNode == this){
                if (l.is_hidden == 0){
                    non_hidden_previous_node = true;
                }
            }
        }
    }
    ...
    for (var i = 0; i < this.links.length; i++) {
        ...
        if (this.type == 'percent' && EDITING == false){
            link_height = link_height * PERCENT_NODE_SCALING;
        }
        ...
        var endnode_height = set[current_link.endNode.key][0];
        if (current_link.endNode.type == 'percent' && EDITING == false){
            endnode_height = endnode_height * PERCENT_NODE_SCALING;
        }
        var end_x = current_link.endNode.x;
        var end_y = current_link.endNode.y - 
                    (endnode_height / 2) * MAX_HEIGHT;

        var end_bottom = end_y + endnode_height * MAX_HEIGHT;

        // if (current_link.endNode.incoming_links.length == 1){
        //     end_y = end_bottom;
        // }

        var link_bottom_left = current_y + link_height * MAX_HEIGHT;
        if (link_bottom_left > bottom_corner){
            new_top = bottom_corner - link_height * MAX_HEIGHT;

            if (new_top >= top_corner){
                current_y = new_top;
            } else {
                current_y = this.y - link_height * MAX_HEIGHT / 2;
            }
        }

        var force_start_height = -1;
        if ((link_height * MAX_HEIGHT) > max_height){
            force_start_height = max_height;
            current_y = this.y - max_height / 2;
        }


        if (this.is_hidden > 0 && non_hidden_previous_node == false){
            context.fillStyle = getDarker(context.fillStyle, 0.7);
            context.strokeStyle = getDarker(context.fillStyle, 0.7);
            current_link.is_hidden = 1;
        } else {
            current_link.is_hidden = 0;
        }

        current_link.draw(context, this.x + WIDTH, current_y, link_height, end_x,
            end_y, end_bottom, force_start_height);
...
UniSankeyLink.prototype.draw = function(context, x, y, height, end_x, end_y, end_node_y = -1,
    ...
    if (force_start_height > -1){
        arc2_start_y = arc_start_y + force_start_height;
        arc2_end_y = arc_end_y + force_start_height;
    }
...
/**
 * UniSankey.getNodeWithKey(key)
 * Return the node with the key key.
 */
UniSankey.prototype.getNodeWithKey = function(key){
    for (var i = 0; i < this.nodes.length; i++){
        for (var j = 0; j < this.nodes[i].length; j++){
            if (this.nodes[i][j].key == key){
...
UniSankey.prototype.drawLegend = function(canvas_id, colour, name, legend_y){
    ...
    var x_coord = LEGEND_PADDING; // canvas.width - LEGEND_WIDTH - HOVER_DIV_WIDTH;
...
UniSankey.prototype.setPositions = function(){
    ...
    for (var column = 0; column < this.nodes.length; column++){
        ...
        var current_max = 0;
        for (var j = 0; j < this.nodes[column].length; j++){
            if (EDITING == true || this.nodes[column][j].is_hidden == 0){
                current_count += 1;
                ...
                for (var i = 0; i < sets.length; i ++){
                    if (sets[i][key][0] > largest_node){
                        ...
                        if (this.nodes[column][j].type == 'percent' && EDITING == false){
                            largest_node = sets[i][key][0] * PERCENT_NODE_SCALING;
                        }
                    }
                }
    ...
    var legend_height_total = this.sets.length * (LEGEND_HEIGHT + LEGEND_PADDING);
    max_height_needed += legend_height_total;
    ...
    for (var column = 0; column < this.nodes.length; column++){
        ...
        var current_y = Math.max(((max_height_needed - legend_height_total) - max_col_height) / 2, 0);
